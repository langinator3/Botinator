...
class Help(formatter.HelpFormatter):
	...
	async def send(self, dest, content=None, embeds=None):
		...
		while True:
			...
			try:
				...
				await page_msg.edit(content=self.bot.bot_prefix + " Quit Help menu.", delete_after=30)
				break
	...
	async def help(self, ctx, *cmds: str):
		...
		if not ctx.message.author.permissions_in(ctx.channel).embed_links:
			return await ctx.send(content=self.bot.bot_prefix + "You don't have permissions to send embeds here. Find a different server/channel where you can embed links and try the help command there.", delete_after=30)
